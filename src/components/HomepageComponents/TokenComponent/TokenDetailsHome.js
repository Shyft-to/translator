...
const TokenDetailsHome = ({nft,cluster}) => {
    ...
    <div>
      <div className={styles.nft_desc_section}>
                ...
                onClick={() =>
                  copyValue(
                    cluster === "mainnet-beta"
                      ? `https://translator.shyft.to/address/${nft.mint}`
                      : `https://translator.shyft.to/address/${nft.mint}?cluster=${cluster}`,
                    true
                  )
                }
              >
        ...
        <div className={styles.nft_image_container_mob}>
          {nft.hasOwnProperty("animation_url") &&
          nft.animation_url !== "" &&
          (nft.animation_url?.endsWith("glb") === true ||
            nft.animation_url?.endsWith("gltf") === true) ? (
            <>
              ...
              <div className="d-flex justify-content-center">
                <button
                  className={`text-center pe-4 ${styles.download_button}`}
                  onClick={() => setShowExpObj(true)}
                >
                  Preview
                </button>
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className={`text-center ${styles.download_button}`}
            ...
            <>
              <motion.img
                src={
                  nft?.hasOwnProperty("image_uri") &&
                  (nft.image_uri === "" ||
                    nft.image_uri.includes("ray-initiative.gift") ||
                    nft.image_uri.includes("dex-ray.gift"))
                    ? unknown
                    : nft.image_uri
                }
                ref={ref3}
              ...
              <div className="d-flex justify-content-center">
                <div className="px-3 pt-2">
                  <div className={styles.view_original_button}>
                    {nft.image_uri !== "" ? (
                      <a href={nft.image_uri} target="_blank" rel="noreferrer">
                        View Original
                      </a>
                    ) : (
                      ""
                ...
                <div className={styles.table_field_value}>
                  {nft.mint !== "" && (
                    <Tooltip
                      ...
                      arrowSize={5}
                    >
                      <button onClick={() => copyValue(nft.mint)}>
                        <img src={copyBtn} />
                      </button>
                    </Tooltip>
                  )}
                ...
                <div className={styles.table_field_value}>
                  {nft.owner !== "" && (
                    <Tooltip
                      ...
                      arrowSize={5}
                    >
                      <button onClick={() => copyValue(nft.owner)}>
                        <img src={copyBtn} />
                      </button>
                    </Tooltip>
                  )}
                  {nft.owner !== "" ? (
                    <a
                      href={
                        cluster === "mainnet-beta"
                          ? `/address/${nft.owner}`
                          : `/address/${nft.owner}?cluster=${cluster}`
                      }
                    >
                      {nft.owner}
                    </a>
            ...
            <div className={`row ${styles.each_row}`}>
              ...
              <div className="col-8">
                <div className={styles.table_field_value}>
                  ...
                  {nft?.collection?.hasOwnProperty("address") &&
                  nft.collection?.address !== "" ? (
                    <a
                      href={
                        cluster === "mainnet-beta"
                          ? `/address/${nft.collection?.address}`
                          : `/address/${nft.collection?.address}?cluster=${cluster}`
                      }
                    >
                      {nft.collection?.name ||
                        collectionName ||
        ...
        {Array.isArray(nft.creators) && nft.creators.length > 0 && (
          <motion.div
            className={`pt-4 ${styles.nft_section}`}
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
            transition={{ delay: 0.8 }}
                      ...
                      <a
                        href={
                          cluster === "mainnet-beta"
                            ...
                            : `/address/${creator.address}?cluster=${cluster}`
                        }
                      >
                        {creator.address ?? "--"}{" "}
                        {creator.verified && (
...
export default TokenDetailsHome;
